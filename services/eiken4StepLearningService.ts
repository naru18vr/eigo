...
export type LearningStepId = 'step-1' | 'step-2' | 'step-3' | 'step-4' | 'step-5' | 'step-6';
...
export const eiken4LearningSteps: LearningStep[] = [
  ...
  {
    id: 'step-2', title: '今と過去のことを言おう', summary: '今していることと、昨日したことを言えるようにしよう。', topics: '現在進行形・過去形・過去進行形', grammarIds: ['present-progressive', 'past-tense', 'past-progressive'],
    lesson: { title: '「今」と「昨日」のしるしを見よう', message: '今している途中なら be動詞 + ing。yesterday があれば、動詞を過去の形にすることが多いよ。', shape: 'I am reading. / I played yesterday.', example: { en: 'I watched TV yesterday.', ja: '私は昨日テレビを見ました。' }, check: { question: 'yesterday がある文は、どれ？', choices: ['I play tennis yesterday.', 'I played tennis yesterday.', 'I am play tennis yesterday.'], answer: 'I played tennis yesterday.', explanation: 'yesterday があるので played のように過去の形にするよ。' } },
  },
  {
    id: 'step-3', title: 'これからのことを言おう', summary: '予定や、できること・してはいけないことを言おう。', topics: '未来の表現・can・must・have to', grammarIds: ['future', 'modal-verb'],
  ...
  {
    id: 'step-4', title: '文をくわしくしよう', summary: '「〜すること」や理由を足して、文を広げよう。', topics: 'to不定詞・動名詞・接続詞・There is', grammarIds: ['infinitive', 'gerund', 'conjunction', 'there-is-are', 'other-eiken4'],
  ...
  {
    id: 'step-5', title: 'くらべて言おう', summary: '2つをくらべたり、一番を言ったりしよう。', topics: '比較級・最上級', grammarIds: ['comparative', 'superlative'],
  ...
  {
    id: 'step-6', title: 'いろいろな問題に挑戦しよう', summary: '覚えたことをまぜて、本番に近い問題をやってみよう。', topics: '単語・会話・読む・聞く・本番形式', grammarIds: [], final: true,
...
type StepLearningData = { steps: Partial<Record<LearningStepId, StepRecord>>; allowedStepIds: LearningStepId[] };
...
const readData = (): StepLearningData => {
  if (typeof localStorage === 'undefined') return { steps: {}, allowedStepIds: [] };
  try {
    const saved = JSON.parse(localStorage.getItem(EIKEN4_STEP_LEARNING_KEY) || '{}') as Partial<StepLearningData>;
    return { steps: saved.steps || {}, allowedStepIds: Array.isArray(saved.allowedStepIds) ? saved.allowedStepIds : [] };
  } catch { return { steps: {}, allowedStepIds: [] }; }
};

const saveData = (data: StepLearningData) => safeSetLearningItem(EIKEN4_STEP_LEARNING_KEY, JSON.stringify(data));
...
export const getLearningStepState = (step: LearningStep, data = readData()): LearningStepState => {
  ...
  if (step.final) return record.completedAt ? 'できた！' : record.startedAt ? 'がんばり中' : 'まだ';
...
export const getLearningStepProgress = (step: LearningStep, data = readData()) => {
  const mastered = allMastered(data);
  return { done: step.final ? Number(Boolean(recordFor(data, step.id).completedAt)) : step.grammarIds.filter(id => mastered.includes(id)).length, total: step.final ? 1 : step.grammarIds.length };
};
