...
export const grade1GrammarItems = GRADE_1_UNITS.flatMap(unit => unit.sentences.map(sentence => ({
...
export const getGrade1DailyItems = (date = localDateKey()) => {
  ...
  const pick = <T extends { word?: string; title?: string; question?: string }>(items: T[], count: number, prefix: string) => items
    ...
    .slice(0, count);
  const result = { words: pick(grade1ReviewWords, 3, 'word'), grammar: pick(grade1GrammarItems, 3, 'grammar') };
