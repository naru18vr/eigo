...
export const getDailyExamQuestions=()=>{const weak=loadWeak();const recent=new Set(getRecentQuestionIds().map(id=>id.replace(/^exam-/,'')));const ranked=eiken4CoreExamQuestions.map((q,i)=>({q,n:hash(`${getLocalDate()}-${i}`),weak:weak[q.id]||0})).sort((a,b)=>b.weak-a.weak||a.n-b.n);const fresh=ranked.filter(item=>!recent.has(item.q.id));return (fresh.length>=10?fresh:ranked).slice(0,10).map(x=>x.q)};
