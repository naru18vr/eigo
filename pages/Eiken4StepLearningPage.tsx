  ...
  return <div className="flex-grow bg-gradient-to-b from-indigo-50 to-white p-4 sm:p-6">
    <main className="mx-auto max-w-xl">
      ...
      {!step.final && <section className="mt-4 rounded-3xl border border-cyan-100 bg-white p-5 shadow-sm">
        ...
        <div className="mt-4 space-y-3">{categories.map(category => <div key={category.id} className="rounded-2xl border border-slate-200 p-4"><div className="flex items-start justify-between gap-3"><div><h3 className="font-extrabold text-slate-900">{category.title}</h3><p className="mt-1 text-sm text-slate-600">{category.description}</p></div><ChevronRightIcon className="h-6 w-6 shrink-0 text-cyan-600"/></div><div className="mt-3 grid gap-2 sm:grid-cols-2">{category.guideTopic && <Button onClick={() => navigate(`/eiken4/grammar-guide?topic=${category.guideTopic}`)} variant="secondary" className="min-h-11 w-full">くわしい説明を見る</Button>}<Button onClick={() => navigate(`/eiken4/grammar-practice/${category.id}`)} className="min-h-11 w-full">この文法を練習する</Button></div></div>)}</div>
      </section>}

      {step.final && <section className="mt-4 rounded-3xl border border-rose-100 bg-white p-5 shadow-sm"><p className="text-xs font-bold text-rose-600">3　まとめに挑戦</p><h2 className="mt-2 text-xl font-extrabold text-slate-900">覚えたことをまぜて復習しよう</h2><p className="mt-2 text-sm leading-6 text-slate-600">まずは習った文法だけをまぜて確認しよう。そのあと今日のおまかせ問題で復習できるよ。</p><Button onClick={startFinalPractice} className="mt-4 w-full" size="lg">いろいろな問題に挑戦</Button><div className="mt-3 grid gap-2 sm:grid-cols-2"><Button onClick={() => navigate('/eiken4/words')} variant="secondary" className="min-h-11 w-full">英単語を練習</Button><Button onClick={() => navigate('/eiken4/listening-practice')} variant="secondary" className="min-h-11 w-full">聞く問題を練習</Button></div></section>}

      <div className="mt-5 rounded-2xl bg-slate-100 p-4 text-sm text-slate-700"><p className="font-bold">次にすること</p><p className="mt-1">{step.final ? 'まずは、習ったことをまぜて確かめよう。' : '確認問題のあと、下の文法を1つずつ練習しよう。'}</p></div>
