...
import type { GrammarCategoryId } from '../services/eiken4GrammarPracticeService';
...
const practiceCategoryByTopic: Record<string, GrammarCategoryId> = {
  past: 'past-tense', future: 'future', infinitive: 'infinitive', gerund: 'gerund', comparison: 'comparative', modal: 'modal-verb', conjunction: 'conjunction', give: 'other-eiken4', there: 'there-is-are',
};
      ...
      <div className="mt-5 space-y-3">{topics.map((topic,index) => {
        ...
        return <article key={topic.id} className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
          ...
          {open && <div className="border-t border-slate-100 p-5">
            ...
            <div className="mt-5 rounded-xl border-2 border-violet-200 bg-violet-50 p-4"><p className="text-xs font-bold text-violet-600">1問チェック</p><p className="mt-1 font-bold text-slate-800">{topic.quiz.question}</p><div className="mt-3 grid gap-2">{topic.quiz.choices.map(choice=><button key={choice} disabled={Boolean(picked)} onClick={()=>setAnswers(value=>({...value,[topic.id]:choice}))} className={`min-h-11 rounded-lg border p-3 text-left font-semibold ${picked && choice===topic.quiz.answer?'border-emerald-500 bg-emerald-50 text-emerald-800':picked===choice?'border-rose-500 bg-rose-50 text-rose-800':'border-violet-200 bg-white text-slate-700'}`}>{choice}</button>)}</div>{picked&&<div className={`mt-3 rounded-lg p-3 ${correct?'bg-emerald-100':'bg-amber-100'}`}><p className="font-bold">{correct?'正解！':`正解：${topic.quiz.answer}`}</p><p className="mt-1 text-sm">{topic.quiz.explanation}</p><button onClick={()=>setAnswers(value=>{const next={...value};delete next[topic.id];return next;})} className="mt-2 text-sm font-bold text-indigo-700">もう一度解く</button></div>}<Button onClick={()=>navigate(`/eiken4/grammar-practice?category=${practiceCategoryByTopic[topic.id]}`)} className="mt-4 w-full">この文法を練習する</Button></div>
          </div>}
        </article>;
      })}</div>
