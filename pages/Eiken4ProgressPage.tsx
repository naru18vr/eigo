...
import { createSevenDayStudyPlan, getStudyPhases } from '../services/eiken4StudyPlanService';
...
const Eiken4ProgressPage: React.FC = () => {
  ...
  const listeningCause = getTopListeningCause();
  const studyPlan = getStudyPhases(examDate);
  const [copyStatus, setCopyStatus] = useState<'idle'|'report'|'plan'|'error'>('idle');
  ...
  const sevenDayPlan = JSON.stringify(createSevenDayStudyPlan(examDate, window.location.href), null, 2);
  const copyReport = async () => { const copied=await copyTextToClipboard(weeklyMessage); setManualCopyText(copied?'':weeklyMessage); setCopyStatus(copied?'report':'error'); };
  const copyPlan = async () => { const copied=await copyTextToClipboard(sevenDayPlan); setManualCopyText(copied?'':sevenDayPlan); setCopyStatus(copied?'plan':'error'); };
  return <div className="flex-grow container mx-auto p-4 sm:p-6 max-w-2xl"><Button onClick={() => navigate('/eiken4')} variant="ghost" size="sm"><ArrowLeftIcon className="h-5 w-5 mr-2"/>英検4級に戻る</Button>
    ...
    <section className="mt-4 rounded-xl bg-white p-5 shadow"><p className="text-xs font-bold text-violet-600">試験日から逆算</p><h2 className="mt-1 text-xl font-extrabold">今は「{studyPlan.current.title}」</h2><p className="mt-2 text-sm text-slate-600">{studyPlan.current.focus}</p><div className="mt-3 flex flex-wrap gap-2">{studyPlan.current.daily.map(item=><span key={item} className="rounded-full bg-violet-50 px-3 py-1 text-xs font-bold text-violet-800">{item}</span>)}</div><details className="mt-4 text-sm"><summary className="cursor-pointer font-bold text-violet-700">9月までの4段階を見る</summary><div className="mt-3 space-y-2">{studyPlan.phases.map(phase=><div key={phase.id} className={`rounded-xl border p-3 ${phase.id===studyPlan.current.id?'border-violet-300 bg-violet-50':'border-slate-200'}`}><b>{phase.title}</b><p className="text-xs text-slate-500">{phase.period}</p><p className="mt-1 text-sm">{phase.focus}</p></div>)}</div></details></section>
    <section className="mt-4 rounded-xl bg-white shadow p-5"><h2 className="font-bold text-slate-800">復習中の苦手分野</h2>{weak.length ? <div className="mt-3 space-y-3">{weak.map(([name,count]) => <div key={name}><div className="flex justify-between text-sm"><span>{name}</span><strong>{count}問</strong></div><div className="h-2 bg-slate-100 rounded mt-1"><div className="h-full bg-amber-500 rounded" style={{width:`${Math.min(100,count*12)}%`}}/></div></div>)}</div> : <p className="text-sm text-emerald-700 mt-3">現在、復習待ちの問題はありません。</p>}{listeningCause && <div className="mt-4 rounded-xl bg-indigo-50 p-3 text-sm text-indigo-900"><b>聞き取りの重点：</b>{listeningCause}</div>}</section>
    ...
    <section className="mt-4 rounded-xl bg-amber-50 p-5"><h2 className="font-bold text-amber-900">保護者向け週間報告</h2><pre className="mt-3 whitespace-pre-wrap rounded-lg bg-white p-3 text-xs text-slate-700">{weeklyMessage}</pre><Button onClick={copyReport} className="mt-3 min-h-12 w-full">週間報告をコピー</Button>{copyStatus==='report'&&<p role="status" className="mt-2 font-bold text-emerald-700">✓ 週間報告をコピーしました。</p>}{transfer.isLong && <p className="mt-2 text-xs font-bold text-amber-800">リンクが長いため、Google Chatで末尾までリンク色になっているか確認してください。</p>}</section>
    <section className="mt-4 rounded-xl border border-violet-200 bg-violet-50 p-5"><p className="text-xs font-bold text-violet-700">別の学習計画アプリへ</p><h2 className="mt-1 font-extrabold text-violet-950">今後7日分の英検タスク</h2><p className="mt-2 text-sm text-violet-900">1日60分以内の5タスクを作り、土曜は類題プリントをミニ模試に置き換えます。重複しないID付きJSONです。</p><Button onClick={copyPlan} className="mt-3 min-h-12 w-full">7日分の学習計画をコピー</Button>{copyStatus==='plan'&&<p role="status" className="mt-2 font-bold text-emerald-700">✓ 学習計画をコピーしました。</p>}</section>
    {copyStatus==='error'&&<section className="mt-4 rounded-xl border border-rose-200 bg-rose-50 p-4"><p role="alert" className="font-bold text-rose-700">× 自動コピーできませんでした。下の文章を長押ししてコピーしてください。</p><textarea readOnly value={manualCopyText} className="mt-2 h-40 w-full rounded-lg border border-slate-300 bg-white p-3 text-xs" aria-label="手動コピー用の文章"/></section>}
