...
const Grade1DailyReviewPage: React.FC = () => {
  ...
  if (!current) return <div className="flex-grow container mx-auto p-4 max-w-xl"><div className="mt-12 rounded-2xl bg-white shadow-xl p-7 text-center"><p className="text-emerald-700 font-bold">中1おさらい完了！</p><h1 className="text-3xl font-bold mt-2">今日の6問が終わりました</h1><p className="text-slate-600 mt-3">単語3語＋文法3問を確認しました。</p><Button onClick={() => navigate('/eiken4/daily')} className="w-full mt-6">次は「今日の15分」へ</Button><Button onClick={() => navigate('/eiken4/course')} variant="ghost" className="w-full mt-2">コース一覧を見る</Button></div></div>;
