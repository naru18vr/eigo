...
const Eiken4WorksheetPage: React.FC = () => {
  ...
  const progress = useMemo(() => parseWorksheetShareData(params.get('data')), [params]);
  const [status, setStatus] = useState<'idle' | 'making' | 'error'>('idle');

  if (!progress) return <div className="flex-grow container mx-auto p-4 max-w-xl"><div className="mt-12 rounded-2xl bg-white shadow-xl p-7 text-center"><h1 className="text-2xl font-bold text-slate-800">リンクを読み込めませんでした</h1><p className="text-slate-600 mt-3">Google Chatのリンクを最後まで選んで、もう一度開いてください。</p><Button onClick={() => navigate('/eiken4')} className="w-full mt-6">英検4級ホームへ</Button></div></div>;
  ...
  const download = async () => {
    setStatus('making');
    try { await downloadDailyWorksheet(progress); setStatus('idle'); }
  ...
  return <div className="flex-grow container mx-auto p-4 sm:p-6 max-w-xl">
    <div className="mt-8 rounded-2xl bg-white shadow-xl border border-emerald-100 p-7 text-center">
      ...
      <Button onClick={download} disabled={status === 'making'} className="w-full mt-6">
        {status === 'making' ? 'PDFを作成中…' : '今日の類題プリントPDF'}
      </Button>
