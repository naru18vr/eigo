...
const Eiken4ListeningFocusPage: React.FC = () => {
  ...
  return <div className="flex-grow bg-slate-50 p-4"><main className="mx-auto max-w-xl"><Button onClick={() => navigate('/eiken4')} variant="ghost" size="sm"><ArrowLeftIcon className="mr-2 h-5 w-5"/>戻る</Button><header className="mt-4 rounded-3xl bg-indigo-700 p-5 text-white"><p className="text-xs font-bold text-indigo-200">あなたの聞き取り原因</p><h1 className="mt-1 text-xl font-extrabold">{cause}</h1><p className="mt-2 text-sm">{tipFor(cause)}</p></header><section className="mt-4 rounded-3xl bg-white p-5 shadow"><div className="flex justify-between text-sm font-bold text-indigo-700"><span>原因別6問</span><span>{index+1}/6</span></div><button disabled={plays>=2} onClick={()=>{speakText(current.audioText,'en-US',plays===0?.72:.88);setPlays(value=>value+1)}} className="mt-4 flex w-full items-center justify-center rounded-xl bg-indigo-600 py-4 font-bold text-white disabled:bg-slate-400"><SpeakerWaveIcon className="mr-2 h-6 w-6"/>音声を聞く（あと{2-plays}回）</button><h2 className="mt-5 text-xl font-bold">{current.question}</h2><div className="mt-4 grid gap-2">{current.choices.map(choice=><button disabled={checked} key={choice} onClick={()=>setSelected(choice)} className={`min-h-12 rounded-xl border p-3 text-left font-bold ${checked&&choice===current.answer?'border-emerald-400 bg-emerald-50':selected===choice?'border-indigo-400 bg-indigo-50':'border-slate-200'}`}>{choice}</button>)}</div>{checked&&<div className={`mt-4 rounded-xl p-4 ${correct?'bg-emerald-50':'bg-rose-50'}`}><b>{correct?'正解！':`正解：${current.answer}`}</b><p className="mt-2 whitespace-pre-line text-sm">{current.transcript}</p></div>}<Button disabled={!selected} onClick={next} className="mt-5 w-full">{checked?'次へ':'答える'}</Button></section></main></div>;
};
