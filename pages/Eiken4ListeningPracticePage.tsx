...
import type { Eiken4ListeningQuestion } from '../data/eiken4Listening';
import { getListeningSectionRanges, getListeningSectionTitle, loadListeningQuestions } from '../services/eiken4QuestionLoader';
...
const Eiken4ListeningPracticePage: React.FC = () => {
  ...
  const sections = useMemo(() => getListeningSectionRanges(questionBank?.length || 0), [questionBank]);
  const questions = useMemo(() => section === null || !questionBank ? [] : questionBank.slice(sections[section].from, sections[section].to), [questionBank, section, sections]);
  ...
  useEffect(() => {
    let active = true;
    setLoadError(false);
    loadListeningQuestions().then(module => { if (active) setQuestionBank(module.eiken4ListeningQuestions); }).catch(() => { if (active) setLoadError(true); });
    return () => { active = false; };
  }, []);
