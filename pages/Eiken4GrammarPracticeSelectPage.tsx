...
const Eiken4GrammarPracticeSelectPage: React.FC = () => {
  ...
  const openCategory = (category: typeof categories[number]) => {
    if (studiedGrammarIds.has(category.id)) {
      navigate(`/eiken4/grammar-practice/${category.id}`);
      return;
    }
    if (category.guideTopic) navigate(`/eiken4/grammar-guide?topic=${category.guideTopic}`);
  };
