...
import { getGrammarCategory, getGrammarPracticeQuestions, recordGrammarPracticeAnswer, saveGrammarPracticeResult, type GrammarCategoryId } from '../services/eiken4GrammarPracticeService';
import { speakText } from '../services/speechService';
import type { DailyAnswer, DailyQuestion } from '../services/eiken4DailyService';
...
const Eiken4GrammarPracticePage: React.FC = () => {
  ...
  const categoryId = new URLSearchParams(location.search).get('category') as GrammarCategoryId | null;
  const category = getGrammarCategory(categoryId);
  const [attemptId, setAttemptId] = useState(makeAttemptId);
  const [questions, setQuestions] = useState<DailyQuestion[]>(() => category ? getGrammarPracticeQuestions(category.id, attemptId) : []);
  ...
  if (!category) return <div className="flex-grow bg-slate-50 p-4"><main className="mx-auto max-w-xl"><Button onClick={() => navigate('/eiken4/grammar-practice-select')} variant="ghost" size="sm"><ArrowLeftIcon className="mr-2 h-5 w-5" />文法を選ぶ</Button><section className="mt-12 rounded-3xl bg-white p-7 text-center shadow"><h1 className="text-2xl font-extrabold">練習する文法を選んでください</h1><p className="mt-3 text-slate-600">文法一覧から、習ったところを選ぶと問題を始められます。</p><Button onClick={() => navigate('/eiken4/grammar-practice-select')} className="mt-6 w-full">文法を選ぶ</Button></section></main></div>;
